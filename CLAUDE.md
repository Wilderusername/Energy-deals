...
- Bump `CACHE_NAME` in `service-worker.js` (currently `canspot-cache-v3`; next bump would be `v4`) whenever the cached app-shell files — including `deals.json` — change meaningfully, so returning visitors pick up the update instead of serving a stale cache. The `activate` handler deletes old-named caches automatically.
