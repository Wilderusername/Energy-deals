...
## 2026-09-06 (70) — Neue Sortierung "Am besten bewertet" (Bayesian Average statt reinem Durchschnitt)

**Analyse vorab**: Die Sortierung (`render()`, `if(sort.value==="price"){...} else if(...)`-Kette) arbeitet auf dem gefilterten `deals`-Array und sortiert per `.sort((a,b)=>...)` - ein neuer Zweig für `sort.value==="rating"` fügt sich dort nahtlos ein. Die Bewertungsdaten selbst kommen ausschließlich aus dem bereits bestehenden `getProductRatingSummary(productId)` (siehe (66)/(69): `percentPositive`, `count`, `criteria`-Sterne je Kriterium) - es wurde bewusst KEINE zusätzliche, separat erfundene Kennzahl für die Sortierung erzeugt, wie explizit gefordert. Das Sortier-Sheet (`#sortOverlay`) sowie der zugrunde liegende `<select id="sort">` sind dieselbe geteilte Komponente für Mobile und Desktop/Webapp - die neue Option wurde daher nach demselben bereits etablierten Muster wie `.detail-rating`/`.community-rating` per `@media(max-width:540px)` NUR auf Mobile sichtbar/wählbar gemacht, obwohl die Sortierlogik selbst allgemeingültig ist.

**Umsetzung**:
- **Bayesian-Average-Formel** (`getWeightedRatingScore(productId)`, neu): `Score = (v/(v+m))·R + (m/(v+m))·C` - R = `avgStars` (neu in `getProductRatingSummary()` ergänzt: Mittel der vier Kriterien Geschmack/Süße/Intensität/Nachgeschmack, exakt dieselbe Zahl, die im Community-Popover als Sterne zu sehen ist), v = `count` (ebenfalls bereits angezeigt), C = `getGlobalAvgRatingStars()` (Durchschnitt von `avgStars` über den GESAMTEN Produktkatalog, einmalig berechnet und gecacht, da `products` sich nach dem Laden nicht mehr ändert), m = `RATING_BAYESIAN_MIN_VOTES = 50` (bewusst im unteren Drittel der erzeugten `count`-Spanne 12-331 gewählt, siehe Kommentar im Code) - Produkte mit wenigen Bewertungen werden dadurch spürbar Richtung Gesamtdurchschnitt gezogen, Produkte mit hunderten kaum noch. Anhand des Beispiels aus der Anfrage durchgerechnet (5,0★/1 Bewertung vs. 4,8★/500 vs. 4,7★/2000) landet das Produkt mit nur 1 Bewertung erwartungsgemäß AM WEITESTEN UNTEN, nicht oben.
- **Sortierung selbst unverändert von der Anzeige getrennt**: Die Bewertungs-Badges/-Popover auf Karte und Detailansicht zeigen weiterhin ausschließlich die echten `percentPositive`/`avgStars`-Werte aus `getProductRatingSummary()` - der gewichtete Score wird ausschließlich innerhalb des `.sort()`-Aufrufs verwendet und nirgends angezeigt (per Vergleich Badge-Text vs. `getWeightedRatingScore()`-Rückgabewert bestätigt, siehe Verifikation).
- **Zukunftsfähig für echte Daten**: Die Formel setzt nur R/v/C voraus, keine bestimmte Datenquelle - sobald `getProductRatingSummary()` künftig echte, aggregierte Nutzerbewertungen statt der deterministischen Demo-Werte liefert, funktioniert `getWeightedRatingScore()` unverändert weiter.
- **UI**: Neue Zeile "Am besten bewertet" in `#sortOverlay` (nach "Endet bald") sowie passendes `<option value="rating">` im zugrunde liegenden `<select id="sort">` (ohne dieses Option-Element hätte `sort.value = "rating"` beim Klick stillschweigend fehlgeschlagen, da ein natives `<select>` beim Setzen eines nicht existierenden Options-Werts auf "" zurückfällt) sowie Eintrag in `sortLabels`. Bestehendes Sortier-Design (Zeilen-Stil, Checkmark, Abstände) unverändert wiederverwendet, keine neue UI gebaut.
- **Nur mobil sichtbar**: `.sort-row[data-sort="rating"]{display:none}`, erst ab `@media(max-width:540px)` sichtbar - Desktop/Webapp kann die neue Option dadurch gar nicht auswählen. Da die neue Zeile im DOM nach "Endet bald" folgt, wäre "Endet bald" auf Desktop (wo "Am besten bewertet" unsichtbar bleibt) nicht mehr `:last-child` und hätte fälschlich eine neue Trennlinie bekommen - dafür `.sort-row[data-sort="ending"]{border-bottom:0}` als Basisregel ergänzt und nur mobil (wo "Am besten bewertet" tatsächlich die letzte sichtbare Zeile ist) wieder zurückgesetzt.
- `CACHE_NAME` in `service-worker.js` auf `canspot-cache-v116` erhöht (Pflichtregel).

**Verifiziert** (mobil, 375px): Neue Zeile erscheint korrekt im Sortier-Sheet, Auswahl setzt `sort.value="rating"` und sortiert die Liste nachweislich absteigend nach `getWeightedRatingScore()` (innerhalb jeder Wochen-Gruppe sowie innerhalb "Einzelartikel"/"Bundles" separat geprüft - die einzige Abweichung von strikt absteigender Reihenfolge über die Gesamtliste ist die bereits bestehende, unabhängige "Bundles immer ans Ende"-Regel, keine Fehlfunktion der neuen Sortierung). Angezeigte Badge-Werte bleiben nachweislich unverändert die echten `percentPositive`-Werte, nicht der interne Score. Bei 1200px Breite (Desktop) ist die neue Zeile per `getComputedStyle` vollständig `display:none`, "Endet bald" zeigt korrekt wieder `border-bottom:0px` wie vor dieser Änderung - Sortier-Sheet auf Desktop optisch unverändert. Keine Konsolenfehler.
...
**Umsetzung**:
- **Entwurf (`pending`)**: Neue, pro Öffnen der Detailansicht frisch aus dem zuletzt gespeicherten Stand initialisierte lokale Variable in `renderProductRating()`. Stern-Taps ändern nur noch `pending[critKey]` (+ Neuzeichnen der Sterne) statt direkt `productRatings`/`saveProductRatings()` aufzurufen - die eigentliche Daumen-Bewertung (👍/👎) bleibt bewusst unverändert sofort-speichernd, da nur die erweiterte Bewertung von der Anfrage betroffen ist.
- **Button**: Neuer `#pdRatingSaveBtn` ("Bewertung speichern") als zusätzliches Element unterhalb der vier Kriterien-Zeilen, noch innerhalb desselben `.rating-adv-inner`-Wrappers (animiert daher zusammen mit dem restlichen Akkordeon, siehe (63) - keine neue Ein-/Ausblendlogik nötig). Dafür `#pdRatingAdvInner` (bisher identisch mit `.rating-adv-inner`) in einen inneren Container für nur die Kriterien-Liste aufgeteilt, der Button liegt als Geschwister daneben, damit er beim Neuzeichnen der Kriterien-Liste nicht mitgelöscht wird.
- **Gestaltung**: Bewusst kein `.btn.primary`, sondern ein kompakter, gedämpfter Sekundär-Look (`border-subtle` + `bg-surface-sunken` + `radius-sm`, kleine 12px-Schrift) - hebt sich als antippbar ab, ohne den Kriterien die Aufmerksamkeit zu nehmen. Standardmäßig `disabled` (dazu `opacity:.45`), solange kein Kriterium in `pending` einen Wert >0 hat - verhindert, dass versehentlich eine leere Bewertung gespeichert wird. Nach dem Speichern wechselt der Button in einen dezenten Erfolgs-Zustand (`.saved`-Klasse, dieselben `--status-success-*`-Farben wie z.B. `.alert-active`, damit Light/Dark automatisch stimmig sind) mit Häkchen-Icon (`#i-check`) und Text "Gespeichert" - keine Animation/kein Popup. Jede weitere Sternänderung setzt den Button sofort zurück auf "Bewertung speichern" (unsaved), da der Entwurf dann wieder vom gespeicherten Stand abweicht.
- Öffnet der Nutzer die Detailansicht erneut, wird `pending` frisch aus dem gespeicherten Stand aufgebaut und der Button startet passend direkt im "Gespeichert"-Zustand, falls mindestens ein Kriterium bereits einen Wert hat.
- `CACHE_NAME` in `service-worker.js` auf `canspot-cache-v114` erhöht (Pflichtregel).

**Verifiziert** (mobil, 375px, Dark UND Light Mode): Button startet deaktiviert/unauffällig, solange keine Sterne gewählt sind; Klick auf einen Stern aktiviert ihn, `localStorage["canspot-ratings"]` bleibt dabei noch unverändert (per Vergleich vor/nach Klick bestätigt); erst Klick auf "Bewertung speichern" schreibt tatsächlich in `localStorage` UND zeigt "✓ Gespeichert"; Klick auf "Speichern", während er deaktiviert ist (kein Kriterium gewählt), verändert `localStorage` nachweislich nicht. Erneutes Ändern eines Sterns nach dem Speichern setzt den Button wieder auf "Bewertung speichern" zurück, ohne den bereits gespeicherten Stand in `localStorage` zu verändern, bis erneut gespeichert wird. Schließen+Wiederöffnen (inkl. vollständigem Reload) zeigt den zuletzt gespeicherten Stand korrekt vorbelegt und startet direkt im "Gespeichert"-Zustand. Layout der vier Kriterien-Zeilen unverändert, Button fügt sich ohne sichtbare Quetschung ein. Bei 1200px Breite bleibt der gesamte Bewertungsbereich (inkl. neuem Button) weiterhin vollständig `display:none`. Keine Konsolenfehler.
...
**Umsetzung**:
- **Badge**: Neuer Button `.community-rating` (Text "Ø 92%") direkt in `.price-line` ergänzt, per `margin-left:auto` an den rechten Rand der Zeile geschoben - Preis und Pfand-Hinweis bleiben davon unberührt (kein `justify-content`-Wechsel auf `.price-line` selbst, der auch die beiden bestehenden Elemente auseinandergezogen hätte). "Ø" statt eines Icons/Symbols gewählt (bereits an anderer Stelle der App als Durchschnitts-Kennzeichnung etabliert, siehe `.avg-context`/"Ø Preis") - macht eindeutig klar, dass es sich um einen Fremd-/Durchschnittswert handelt, ausdrücklich OHNE den Daumen-Icon aus der Detailansicht zu verwenden (der ist dort für die eigene Stimme reserviert). Optik: dezente "Pille" (`bg-surface-sunken` + `border-subtle` + `radius-pill`, gedämpfte `text-secondary`-Farbe) - dieselbe Sprache wie `.rating-summary` in der Detailansicht, keine grelle Akzentfarbe, funktioniert automatisch in Light/Dark, da ausschließlich themenfähige CSS-Variablen verwendet werden.
- **Popover**: Klick auf das Badge öffnet `#communityRatingPopover`, eine neue, eigene Sprechblase nach exakt demselben Prinzip wie das bereits bestehende `#pricePopover` ("Zuletzt geprüft", Klick auf den Preis) - fixed positioniert unterhalb des geklickten Badges, mit Pfeil, geklemmt an den Viewport-Rand, schließt bei Klick außerhalb/Scroll/Resize, erneuter Klick auf dasselbe Badge schließt wieder (Toggle). Eigene Instanz statt Wiederverwendung von `#pricePopover`, da der Inhalt strukturiert ist (Titel, Prozent+Anzahl, 4 Kriterien-Zeilen) statt reinem Text. Inhalt: "Community-Bewertung"-Titel, `X% positiv · Y Bewertungen` sowie die vier Kriterien aus (63) (Geschmack/Süße/Intensität/Nachgeschmack) als reine, nicht-interaktive `<svg>`-Sterne (kein `<button>`, keine Klick-Handler) - bewusst rein informativ, es gibt dort keine Möglichkeit für den Nutzer, selbst zu bewerten. Die Kriterien-Durchschnittswerte sind neu (`criteria` in `getProductRatingSummary()` ergänzt, ebenfalls deterministisch je Kriterium via `hashStr()`, Bereich 3,0-5,0, gerundet auf ganze Sterne für die Anzeige).
- **Nur mobil**: `.community-rating{display:none}` als Basis-Regel AUSSERHALB der bestehenden Mobile-Card-Media-Query (`@media max-width:540px`, dort dann `display:inline-flex`) - ohne dieses Gegenstück wäre der `<button>` auf Desktop-Breiten mit bloßen Browser-Standardstilen sichtbar geblieben, statt wie gefordert komplett zu verschwinden. Popover selbst benötigt keine eigene Sperre, da sein einziger Auslöser (das Badge) auf Desktop ohnehin unsichtbar/unklickbar ist.
- `CACHE_NAME` in `service-worker.js` auf `canspot-cache-v112` erhöht (Pflichtregel).

**Verifiziert** (mobil, 375px, Dark UND Light Mode, `localStorage` geleert): Badge erscheint rechtsbündig in der Preiszeile, Preis/Pfand-Position und -Abstand unverändert, kein Zeilenumbruch. Klick öffnet die Sprechblase mit korrektem Prozent-/Anzahl-/Sterne-Inhalt (Werte identisch zum entsprechenden `#pdRatingSummary`-Wert in der Detailansicht desselben Produkts, per Vergleich bestätigt); erneuter Klick auf dasselbe Badge sowie Klick außerhalb schließen sie wieder. Bestehendes `#pricePopover` (Klick auf den Preis selbst), Favoriten-Herz und Produkt-Detail-Klick auf derselben Karte weiterhin unverändert funktionsfähig (keine Event-Konflikte). Bei 1200px Breite ist das Badge per `getComputedStyle` vollständig `display:none`, Preiszeile optisch identisch zum Zustand vor dieser Änderung (Screenshot-Vergleich). Keine neuen Konsolenfehler (die weiterhin sichtbaren `navigator.vibrate`-Meldungen sind unverändert das bereits dokumentierte Testartefakt aus (64), unabhängig von dieser Änderung).

**Hinweis zum Umfang**: Ausschließlich Mobile-Version bearbeitet und verifiziert, wie seit [[feedback-scope-mobile-only-default]] festgelegt.

---

## 2026-09-06 (65) — Gesamtbewertung + Preisalarm-Glocke in der Produkt-Detailansicht

**Analyse vorab**: Die Hero-Box (`.detail-hero`) hatte bisher zwei Ecken belegt (Zurück oben links, Teilen oben rechts) - unten rechts war freier Raum in der abgerundeten Box, den der Nutzer als "runden Bereich" für eine neue Glocke vorgesehen hat. Die Bewertungszeile (`.rating-quick`: Daumen + "Erweitert bewerten") hatte nach (62)/(63) bereits Platz für eine kleine dritte Komponente, solange sie kompakt bleibt. Für den Preisalarm existierte bereits ein vollständiges, funktionierendes System (`priceAlerts[productId]` in `localStorage["canspot-alerts"]`, `savePriceAlerts()`, `updateAlertsDot()`, `.alert-box`/`.alert-input-row`/`.alert-active`-Stile) im Preisverlauf-Sheet (`histOverlay`) - für die neue Glocke wurde dieselbe Datenbasis/dieselben Hilfsfunktionen wiederverwendet, aber ein eigener, unabhängiger Bedienpfad (eigene IDs `pdAlert*`) ergänzt, damit beide Instanzen (histOverlay bleibt unverändert) sich nicht gegenseitig stören.

**1. Kompakte Gesamtbewertung**: Neue kleine "Pille" `#pdRatingSummary` (Daumen-hoch-Icon + Prozentzahl, z.B. "79 %") direkt zwischen den beiden Daumen-Buttons und "Erweitert bewerten" in derselben, bereits bestehenden Zeile (`.rating-quick`) eingefügt - keine zusätzliche Zeile, keine neuen Abstände. Da es in diesem Demo-Prototyp keine echten Mehrnutzer-Bewertungsdaten gibt (siehe `deals.json` `_meta.description`), wird der Wert wie andere Demo-Kennzahlen der App (`checkedMinutesAgo`, `genPriceHistory`) deterministisch aus der Produkt-ID erzeugt (`getProductRatingSummary()`, per `hashStr()`) - stabil über Reloads, unabhängig von der eigenen Daumen-Stimme des aktuellen Nutzers (Anzahl der Bewertungen steckt nur im `aria-label`, nicht sichtbar, um die Zeile kurz zu halten). Optisch bewusst dieselbe kompakte "Pillen"-Sprache wie z.B. `.saving-percent`, in gedämpftem `text-muted` statt einer eigenen Akzentfarbe, damit sie neben den Daumen nicht dominiert.
**2. Preisalarm-Glocke**: Neuer Button `#pdAlertBtn` (`#i-bell`, bereits vorhandenes Icon) unten rechts im Hero-Bild ergänzt, spiegelbildlich zu "Zurück" oben links, gleicher `.btn`-Stil wie die bestehenden Ecken-Buttons. Tippen blendet `#pdAlertPanel` darunter smooth ein/aus (identische Grid-Höhen-/Opacity-Technik wie `.rating-adv`, siehe (63) - eigene, gleich aufgebaute CSS-Klasse `.detail-alert-panel`/`-inner` statt fachfremder Wiederverwendung der `rating-*`-Klassen). Das Panel sitzt bewusst NACH der Bewertung, VOR "Nährwerte" (Prioritätsreihenfolge aus der Anfrage: Bewertung vor "zusätzlichen Funktionen wie Preisalarm"). Inhalt: derselbe Zielpreis-Eingabefluss wie im Preisverlauf-Sheet (`.alert-box`, Eingabefeld + "Preisalarm aktivieren"), schreibt in denselben `priceAlerts`-Speicher und ruft dieselben Hilfsfunktionen (`savePriceAlerts()`, `updateAlertsDot()`, `showToast()`, `attachGermanPriceInput()`, `parseGermanPrice()`) auf. Ist ein Alarm aktiv, erscheint wie gewohnt "Aktiver Alarm bei X €" mit "entfernen"-Link, UND ein kleiner Punkt auf der Glocke selbst (`.alert-dot`, gleiche Farbe/Technik wie der Hinweis-Punkt am Burger-Menü, siehe (61)/(63)) macht den aktiven Alarm bereits sichtbar, ohne das Panel zu öffnen. Panel klappt bei jedem Öffnen der Detailansicht wieder eingeklappt auf (wie das Bewertungs- und Nährwerte-Akkordeon), der Alarm-Status selbst bleibt natürlich erhalten. Ohne aktives Angebot für das Produkt (z.B. bei manchen Einträgen unter "Ähnliche Produkte") bleibt die Glocke ausgeblendet, exakt wie beim bestehenden Teilen-Button.
- Beide neuen Elemente sind wie die gesamte Bewertung aus (62) NUR mobil sichtbar (`@media(max-width:540px)`, gleiches Muster wie `.detail-rating`) - Desktop-/Webapp-Ansicht von `#productDetailOverlay` (dasselbe geteilte Sheet) bleibt unverändert (`display:none`, per Screenshot und `getComputedStyle` bestätigt).
- `CACHE_NAME` in `service-worker.js` auf `canspot-cache-v111` erhöht (Pflichtregel).

**Verifiziert** (mobil, 375px, Dark UND Light Mode, `localStorage` geleert): Bewertungs-Pille zeigt korrekten Prozentwert, bricht in der Zeile nicht um. Glocke öffnet/schließt das Panel smooth (Grid-Werte per `getComputedStyle` bestätigt: 0px/opacity 0 ↔ tatsächliche Inhaltshöhe/opacity 1), Eingabefeld vorbelegt mit Angebotspreis − 0,10 €, "Preisalarm aktivieren" speichert korrekt in `localStorage["canspot-alerts"]`, Punkt auf der Glocke erscheint, "entfernen" löscht Alarm + Punkt korrekt wieder, `updateAlertsDot()` aktualisiert den Alarme-Tab-Badge. Bestehendes Preisalarm-Feature im Preisverlauf-Sheet (`histOverlay`) weiterhin unverändert funktionsfähig und zeigt denselben zugrunde liegenden Zustand. "Erweitert bewerten" unverändert getestet, keine Regression. Bei 1200px Breite sind Glocke, Panel und Bewertungsblock vollständig unsichtbar (`display:none`), restliche Desktop-Ansicht pixelgleich zu vorher (Screenshot-Vergleich). Keine neuen Konsolenfehler (die weiterhin auftretenden `navigator.vibrate`-Meldungen stammen unverändert aus dem Neuigkeiten-Postfach-Swipe aus (64) und sind ein reines Testartefakt synthetischer Touch-Events, siehe dortige Notiz).

**Hinweis zum Umfang**: Ausschließlich Mobile-Version bearbeitet und verifiziert, wie seit [[feedback-scope-mobile-only-default]] festgelegt.

---

## 2026-09-06 (64) — Zweistufiger Swipe im Neuigkeiten-Postfach (Burger-Menü)

**Analyse vorab**: `initNewsSwipe()` (Burger-Menü → Neuigkeiten-Postfach, `#menuNewsInboxList`) clampte die Zeile bisher hart auf `±REVEAL` (76px) und setzte beim Loslassen `row.style.transition = ""` - da für `.news-inbox-row` nirgends eine CSS-`transition` für `transform` definiert ist, geschah dieses "Einrasten" bisher komplett unanimiert/instant, weshalb es sich wie ein abrupter Stopp anfühlte. Die Aktion selbst ließ sich danach ausschließlich über einen zusätzlichen Tap auf die aufgedeckte Fläche auslösen - genau das vom Nutzer beschriebene Problem. Diese Datei/Funktion ist ausschließlich vom mobilen Burger-Menü aus erreichbar (die Desktop-/Webapp-Glocke nutzt `newsOverlay`/`renderNewsList()`/`.news-card` - komplett andere Funktionen ohne Swipe, unangetastet), Scope war also automatisch mobil-only, keine zusätzliche Media-Query-Abgrenzung nötig.

**Umsetzung** (nur `initNewsSwipe()` + eine begleitende CSS-Breitenanpassung geändert, Rest der Funktion/Datenlogik unverändert):
- **Zweistufige Bewegung**: Bis `REVEAL` (76px, unverändert - exakt die bisherige "voll aufgedeckt"-Position) folgt die Zeile weiterhin 1:1 dem Finger. Darüber hinaus setzt jetzt spürbarer, gedämpfter Widerstand ein (`RESISTANCE=0.4`: nur 40% der zusätzlichen Fingerbewegung wird sichtbar), gedeckelt bei einem harten Anschlag (`MAX_EXTRA=46px`) - fühlt sich wie ein natürliches "Reingummi" statt eines toten Stopps an.
- **Einrasten + Commit-Schwelle**: Lässt der Nutzer los, ohne `COMMIT_EXTRA` (70px tatsächliche, ungedämpfte Fingerbewegung über REVEAL hinaus) erreicht zu haben, gleitet die Zeile animiert (neue Snap-Transition, siehe unten) exakt auf `REVEAL` zurück und bleibt dort eingerastet stehen (Symbol vollständig sichtbar) - identisch zum bisherigen Endzustand, nur jetzt smooth statt hart. Wird die Schwelle beim Loslassen überschritten, wird die Aktion sofort ausgeführt (`commitSwipeAction()`): links → Zeile gleitet animiert vollständig aus der Ansicht (`translateX(-100%)`, 220ms) und wird danach gelöscht; rechts → Zeile gleitet zurück auf 0 und der Gelesen-Status wird danach umgeschaltet (kein Entfernen aus der Liste, da "gelesen/ungelesen" kein Lösch-Vorgang ist). Ein Tap auf das eingerastete Symbol funktioniert unverändert als sofortige Alternative zum Weiterwischen (bestehende Klick-Handler nicht angefasst); ein Tap auf die Zeile selbst schließt sie weiterhin nur.
- **Snap-Animation ergänzt**: Neue Transition `transform .25s cubic-bezier(.22,1,.36,1)` wird jetzt explizit beim Loslassen gesetzt (vorher fehlte eine CSS-Regel dafür komplett, siehe Analyse) - macht sowohl das Einrasten auf REVEAL als auch das Zurückgleiten auf 0 sichtbar smooth, während `touchmove` weiterhin `transition:none` für 1:1-Fingerverfolgung behält.
- **Dezentes Threshold-Feedback**: Einmaliger, kurzer `navigator.vibrate()`-Puls (8ms) exakt beim Überschreiten von REVEAL während des Ziehens (pro Richtungswechsel neu scharf geschaltet, kein wiederholtes Vibrieren im Widerstandsbereich danach), plus ein etwas stärkerer Puls (14ms) beim tatsächlichen Commit - rein optional/defensiv über neue Hilfsfunktion `vibrateSafe()` (try/catch + Feature-Check), damit eine von der Plattform verweigerte Vibration (z.B. Browser-Policy) niemals die Swipe-Animation/-Aktion selbst unterbricht.
- **CSS-Begleitanpassung**: `.news-inbox-delete-bg`/`.news-inbox-toggle-read-bg` von `width:76px` auf `width:122px` (REVEAL+MAX_EXTRA) verbreitert, damit die jetzt bis in den Widerstandsbereich hinein sichtbare Fläche weiterhin durchgängig in derselben Farbe erscheint statt eine Lücke zur Sheet-Hintergrundfarbe zu zeigen. Icons selbst bleiben unverändert positioniert (weiterhin an der jeweils verankerten Kante per `padding-right`/`-left`), da die Flächen nur nach hinten/unten unter die Zeile erweitert wurden - im Ruhezustand (Zeile bei 0) optisch kein Unterschied zu vorher.
- `CACHE_NAME` in `service-worker.js` auf `canspot-cache-v110` erhöht (Pflichtregel).

**Verifiziert** (mobil, 375px, per synthetischen Touch-Events durchgespielt, da die Test-Umgebung keine echten Touch-Gesten erzeugen kann): Leichter Swipe (< REVEAL/2) → Zeile springt zurück auf 0. Swipe bis in den Widerstandsbereich (90px Fingerbewegung) → sichtbare Position landet bei den berechneten 81,6px (76 + 14×0,4, Widerstandsformel bestätigt), rastet beim Loslassen exakt auf 76px ein. Swipe über die Commit-Schwelle (200px) → sichtbare Position deckelt korrekt bei 122px (Hartanschlag), beim Loslassen sofortiger Commit: links löscht die Nachricht nach der Slide-off-Animation tatsächlich (per `localStorage["canspot-news-deleted"]` bestätigt), rechts schaltet den Gelesen-Status um (Zeile bleibt erhalten, `localStorage["canspot-news-read"]` bestätigt aktualisiert). Direkter Tap auf das eingerastete Papierkorb-Symbol funktioniert weiterhin unverändert sofort. Auf Desktop-Breite (1200px) bleibt das Burger-Menü (und damit dieses gesamte Postfach) unverändert unerreichbar (`display:none`), Glocke/`newsOverlay` unangetastet. Die während der Tests aufgetretenen `navigator.vibrate`-Konsolenmeldungen sind ein reines Artefakt der synthetischen Test-Events (Chrome blockt Vibration ohne "echten" Nutzer-Tap) und durch `vibrateSafe()` bereits abgefangen - auf einem echten Gerät mit echten Touch-Events tritt das nicht auf.

**Hinweis zum Umfang**: Ausschließlich Mobile-Version bearbeitet und verifiziert, wie seit [[feedback-scope-mobile-only-default]] festgelegt.

---

## 2026-09-06 (63) — Bewertungs-Kriterium ausgetauscht, sanfte Ein-/Ausblend-Animation für "Erweitert bewerten", Hinweis-Punkt am Burger-Menü vereinheitlicht

**1. Kriterium "Preis-Leistung" → "Nachgeschmack"**: In `RATING_CRITERIA` (siehe (62)) den Eintrag `{key:"value", label:"Preis-Leistung"}` durch `{key:"aftertaste", label:"Nachgeschmack"}` ersetzt. Reihenfolge jetzt Geschmack/Süße/Intensität/Nachgeschmack, wie angefragt. Alter `value`-Schlüssel in bereits gespeicherten `canspot-ratings`-Einträgen wird schlicht nicht mehr gerendert (keine Migration nötig, reine Demo-Daten).

**2. Sanfte Ein-/Ausblend-Animation für die erweiterte Bewertung**: `#pdRatingAdv` wechselte bisher hart per `display:none`/`.open{display:block}` - jetzt dieselbe Grid-Höhen-Transition-Technik wie bereits `.store-more-group`/`.period-group` im Projekt (siehe dortige Kommentare: animiert zuverlässig auf die tatsächliche Inhaltshöhe statt eines geschätzten `max-height`-Werts). Dafür einen neuen inneren Wrapper `#pdRatingAdvInner` (`.rating-adv-inner{overflow:hidden}`) ergänzt, der den Inhalt während der Transition clippt, während der äußere `.rating-adv` per `grid-template-rows:0fr→1fr` (0,28s) sanft aufklappt - zusätzlich ein `opacity:0→1`-Fade (0,2s) für den "fließend entfalten"-Effekt statt eines reinen Höhensprungs. `renderProductRating()` schreibt die 4 Kriterien-Zeilen jetzt in `#pdRatingAdvInner` statt direkt in `#pdRatingAdv` (alle betroffenen `querySelectorAll`-Aufrufe entsprechend umgestellt); Verhalten (Klick auf Sterne, Speichern, beim erneuten Öffnen der Detailansicht wieder eingeklappt startend) unverändert.

**3. Hinweis-Punkt am Burger-Menü vereinheitlicht**: `#menuDot` erbte bisher `border-color` unverändert von der geteilten `.iconbtn .dot`-Basisregel (`var(--brand-primary-dark)`, ein dunkles Navy) während `background` bereits auf `var(--brand-primary-light)` (helles Blau) gesetzt war - der 2px-Rahmen erschien dadurch in einer anderen Farbe als der innere Punkt. `border-color:var(--brand-primary-light)` (identisch zu `background`) ergänzt, sodass Rahmen und Punkt exakt dieselbe Farbe haben und als ein einziger, etwas größerer Punkt wirken. Größe (8px), Rahmenbreite (2px, aus der Basisregel), Position und Glow (`box-shadow`) unverändert. Die geteilte `.iconbtn .dot`-Regel selbst (betrifft auch `#notifDot`, die Desktop-/Webapp-Glocke) wurde nicht angefasst.
- `CACHE_NAME` in `service-worker.js` auf `canspot-cache-v109` erhöht (Pflichtregel).

**Verifiziert** (mobil, 375px, Dark Mode, geleerter `localStorage`): Erweiterte Bewertung zeigt jetzt Geschmack/Süße/Intensität/Nachgeschmack in dieser Reihenfolge. Öffnen/Schließen per `computed style`-Prüfung bestätigt: startet bei `grid-template-rows:0px`/`opacity:0`, expandiert auf die tatsächliche Inhaltshöhe (143px) mit `opacity:1` über eine aktive `transition`, Schließen kehrt denselben Übergang um; visuell kein ruckartiger Sprung, Layout darunter (Nährwerte etc.) wird während der Animation nicht verschoben. `#menuDot`: `background` und `border-color` per `getComputedStyle` identisch bestätigt (`rgb(112, 146, 197)` beide), Größe/Glow unverändert, Screenshot zeigt einen einheitlich wirkenden Punkt. Bei 1200px Breite weiterhin `.detail-rating{display:none}` und `.iconbtn-menu{display:none}` (Bell/`#notifBtn` sichtbar) - Desktop-/Webapp-Ansicht vollständig unverändert. Keine Konsolenfehler.

**Hinweis zum Umfang**: Ausschließlich Mobile-Version bearbeitet und verifiziert, wie seit [[feedback-scope-mobile-only-default]] festgelegt.

---

## 2026-09-06 (62) — Bewertungssystem (Daumen + optionale erweiterte Kriterien) in der Produkt-Detailansicht

**Analyse vorab**: Die bestehende Detailansicht (`#productDetailOverlay`) folgt dem Muster Hero-Bild → Titel/Untertitel → Nährwerte (mit eigenem, standardmäßig eingeklapptem "Alle Nährwerte anzeigen"-Akkordeon, `.nutrition-toggle`/`.nutrition-full`) → Ähnliche Produkte. Die Bewertung wurde direkt unter Titel/Untertitel und vor "Nährwerte" platziert (statt als eigener `field-label`-Abschnitt wie "Nährwerte"/"Ähnliche Produkte"), damit sie sich dezent einfügt statt wie ein eigener großer Bereich zu wirken - und das bereits vorhandene Einklapp-Prinzip aus dem Nährwerte-Akkordeon direkt danebenliegend wiederverwendet, statt ein neues UI-Konzept einzuführen.

**Umsetzung**:
- **Schnelle Daumenbewertung**: Zwei kleine runde Icon-Buttons (`#pdThumbUp`/`#pdThumbDown`, neue Sprite-Icons `#i-thumb-up`/`#i-thumb-down`, gleicher Stroke-only-Stil wie alle anderen Icons), nebeneinander, ohne jede Beschriftung. Ein Tap setzt die Bewertung und färbt/füllt das jeweilige Icon (Grün/`--status-success-*` für 👍, Rot/`--status-error-*` für 👎, exakt dieselbe Farb-bei-aktiv-Konvention wie beim bestehenden `.btn.fav.active`-Herz). Beide Optionen schließen sich gegenseitig aus, erneutes Antippen der bereits aktiven Option hebt die Bewertung wieder auf (kein erzwungener Zustand).
- **Erweiterte Bewertung**: Dezenter Text-Link "Erweitert bewerten" mit rotierendem Chevron (gleiche visuelle Sprache wie `.nutrition-toggle`) rechts neben den Daumen in derselben Zeile - nimmt im eingeklappten Zustand keine zusätzliche Zeile ein. Aufgeklappt erscheinen 4 kompakte Zeilen mit je 5 antippbaren Sternen (`i-star` wiederverwendet, `--status-warning-text` als Füllfarbe bei Auswahl): **Geschmack, Süße, Intensität, Preis-Leistung**. Kriterien bewusst produktkategorie-spezifisch gewählt statt eines generischen 1-5-Gesamtsterns: Geschmack + Preis-Leistung sind Standard bei praktisch jeder Getränkebewertung, Süße und Intensität (Koffein-/Wirkungs-"Kick") sind die beiden Eigenschaften, in denen sich Energy Drinks untereinander am meisten unterscheiden und die kaufentscheidend sind - anders als z.B. "Aussehen"/"Mundgefühl" aus Bier-/Wein-Bewertungsschemata, die hier keine Entsprechung hätten. Jeder Stern-Tap speichert sofort (kein separater "Absenden"-Button, kein Formular-Gefühl); erneutes Antippen des bereits gesetzten höchsten Sterns setzt das Kriterium zurück auf unbewertet.
- **Persistenz**: Neuer `localStorage`-Schlüssel `canspot-ratings` (`{ [productId]: { vote, taste, sweetness, intensity, value } }`), [CLAUDE.md](CLAUDE.md) (Persistence-Abschnitt) ergänzt. Panel startet bei jedem Öffnen der Detailansicht wieder eingeklappt (wie das Nährwerte-Akkordeon), gespeicherte Werte bleiben aber erhalten und werden beim erneuten Aufklappen korrekt wiederhergestellt.
- **Ausschließlich mobil**: Da `#productDetailOverlay` dasselbe geteilte Sheet für Mobile und Desktop/Webapp ist, wurde der komplette Bewertungsblock (`.detail-rating`) nach demselben bereits etablierten Muster wie Burger-Menü-Button vs. Glocke (`@media (max-width:540px)`) ein-/ausgeblendet - auf Desktop-Breiten (>540px) `display:none`, komplett unverändertes Layout dort (per `getComputedStyle` und Screenshot-Vergleich geprüft).
- Neue SVG-Icons `#i-thumb-up`/`#i-thumb-down` zum Sprite ergänzt. `CACHE_NAME` in `service-worker.js` auf `canspot-cache-v108` erhöht (Pflichtregel).

**Verifiziert** (mobil, 375px, Dark UND Light Mode, mit geleertem `localStorage`): 👍/👎 schließen sich korrekt gegenseitig aus, erneutes Antippen hebt die Bewertung auf, aktives Icon farbig gefüllt; "Erweitert bewerten" klappt korrekt auf/zu, alle 4 Kriterien-Zeilen vorhanden; Sterne setzen/löschen den jeweiligen Wert korrekt und bleiben über Schließen+Wiederöffnen der Detailansicht erhalten (Panel selbst klappt beim Wiederöffnen wieder ein); `localStorage["canspot-ratings"]` enthält die erwarteten Werte. Bei 1200px Breite ist `.detail-rating` per `display:none` vollständig unsichtbar, restliche Detailansicht (Nährwerte, Ähnliche Produkte, Teilen-Button) unverändert - Screenshot-Vergleich vor/nach bestätigt keine Layoutverschiebung. Keine Konsolenfehler in beiden Breiten/Themes.
...
**Umsetzung**:
- **Name**: `nameBlock` von einem einzelnen `#profileName`-Input auf zwei nebeneinanderliegende Inputs `#profileFirstName`/`#profileLastName` umgestellt (`.field-edit-row.stacked` + `.field-edit-row-inline` - dieselbe bereits bestehende Mehrfeld-Technik wie beim "Adresse"-Block/PLZ+Ort, siehe dort). Neuer Modifier `.field-edit-row-inline.even` für eine 50/50-Aufteilung (PLZ/Ort nutzen bewusst weiterhin die bestehende 35/65-Regel unverändert) statt Vorname/Nachname ungleich breit zu quetschen. Weiterhin EIN gemeinsamer Speicher-Button (`data-save="name"`, wie zuvor) und EIN gemeinsamer Anzeigewert (Vorname+Nachname zusammengefügt) an beiden bisherigen Anzeigestellen (Konto verwalten + "Mein Bereich"-Kopfzeile) - unverändertes Verhalten dort, nur die Eingabe ist jetzt zweigeteilt.
- **Platzhalter statt Beispieldaten**: Alle drei Felder (Name, E-Mail, Geburtsdatum) starten jetzt leer - `value="Max Mustermann"`/`"max.mustermann@example.com"` aus den `<input>`s entfernt. Neue Anzeigetexte, wenn nichts gespeichert ist: "Name eingeben" / "E-Mail-Adresse eingeben" / "TT.MM.JJJJ" (ersetzt "Noch nicht angegeben" beim Geburtsdatum) - alle in einer neuen, absichtlich dezenteren Textfarbe (`.placeholder-text`, `var(--text-disabled)` + reduzierte Schriftstärke) statt der fett/volltonigen Farbe echter gespeicherter Werte, damit auf den ersten Blick erkennbar ist, dass es sich um einen Hinweis und keine echte Angabe handelt. Die `<input>`-eigenen `placeholder`-Attribute ("Vorname eingeben"/"Nachname eingeben"/"E-Mail-Adresse eingeben") verschwinden automatisch beim Tippen - Standardverhalten von HTML, keine eigene Logik nötig. `type="date"` beim Geburtsdatum unterstützt keinen eigenen Text-Platzhalter (browsereigene, nicht stylebare Leerdarstellung) - die "TT.MM.JJJJ"-Anzeige betrifft daher die Vor-dem-Bearbeiten-Ansicht (`#profileBirthdateText`), nicht das Eingabefeld selbst.
- **Avatar-Initialen-Bug vermieden**: `updateAvatarInitials()` bekommt jetzt konsequent den ECHTEN (ggf. leeren) Namen übergeben statt des ggf. angezeigten Platzhaltertexts - eine erste Zwischenversion hätte sonst aus "Name eingeben" fälschlich die Initialen "NE" berechnet. Zeigt bei leerem Namen weiterhin "?" (bereits vorhandener Fallback in der Funktion, unverändert).
- localStorage-Schlüssel `canspot-name` durch `canspot-firstname`/`canspot-lastname` ersetzt; [CLAUDE.md](CLAUDE.md) (Persistence-Abschnitt) entsprechend aktualisiert (dabei auch `canspot-birthdate`/`canspot-address` ergänzt, die dort bisher fehlten, obwohl schon länger in Verwendung).
- `CACHE_NAME` in `service-worker.js` auf `canspot-cache-v106` erhöht (Pflichtregel).

**Verifiziert** (mobil, Dark UND Light Mode, mit komplett geleertem `localStorage` für einen echten Erstnutzer-Zustand): Alle drei Felder zeigen im Ausgangszustand die neuen, dezent gefärbten Platzhaltertexte statt Beispieldaten; "Mein Bereich"-Kopfzeile zeigt passend "Name eingeben" (dezent) mit "?"-Avatar. Bearbeiten öffnet Vorname/Nachname nebeneinander (50/50) mit korrekten Platzhaltern; Speichern von Vorname+Nachname, nur Vorname (Teilangabe) sowie E-Mail/Geburtsdatum einzeln geprüft - Anzeige wechselt jeweils korrekt auf fette, normale Textfarbe, Platzhalter-Klasse wird entfernt, Avatar-Initialen stimmen ("EM" bzw. "A"), Geburtsdatum korrekt im deutschen Format (15.05.1990) formatiert. Persistenz über Reload bestätigt. Bestehende Funktionen (Speichern-Buttons, Bearbeiten-Stift, Auto-Zuklappen beim Wegklicken, Konto-löschen-Bereich, Passwort-Feld) unverändert getestet. Keine Konsolenfehler.
...
**Entscheidung gegen eine komplett neue Seite/Datenstruktur**: Statt eines eigenen Containers mit eigener Karten-Logik wird "Neuheiten" - genau wie "Favoriten" es bereits vormacht - als eigener, unabhängiger Filter (`viewNewOnly`) auf die bestehende `#deals`-Liste umgesetzt. Dadurch bekommen die Karten automatisch exakt dasselbe Design/Verhalten (Favorisieren, Preisverlauf, Kartenansicht, Marken-Chips, Sortierung, "Bester Preis"/"Neu"-Badges) wie überall sonst in der App - ohne Karten-Code zu duplizieren. Bewusst eine **neue, eigene** Variable statt Wiederverwendung von `sort.value==="new"`: letzteres ist ein einzelner globaler Zustand, der auch direkt im Sortier-Sheet wählbar ist - hätte der Tab stattdessen `sort.value` gesetzt, wäre die Einschränkung beim Wechsel zu Favoriten/Alarme (deren Handler laut Auftrag nicht verändert werden durften) fälschlich bestehen geblieben.

**Umsetzung**:
- Bottom-Nav-Button `navSearch`/"Suche" ersetzt durch `navNeu`/"Neuheiten" (`data-nav="neu"`), Icon `#i-sparkle` wiederverwendet (dasselbe Icon, das die bereits bestehende "Neuheiten"-Ziehharmonika im Burger-Menü nutzt - passend zum bestehenden Design, kein neues Icon nötig).
- Neue Variable `viewNewOnly` (analog zu `viewFavoritesOnly`), in `render()` als zusätzlicher Filter (`filtered.filter(d=>d.isNew)`) ergänzt.
- `navNeu`-Click-Handler (ersetzt den alten `navSearch`-Handler 1:1 an derselben Stelle) setzt `viewNewOnly=true`/`viewFavoritesOnly=false` und ruft dieselbe `showDealsView()` wie Favoriten/Start auf.
- Einzige notwendige Anpassung an bestehenden Handlern: `navHome` und `navFav` setzen jetzt zusätzlich `viewNewOnly=false` zurück (je eine Zeile) - notwendig, damit die neue Einschränkung nicht in Favoriten/Start "durchsickert", wenn man vorher auf "Neuheiten" war. `navAlerts`/`navProfile` bewusst NICHT angefasst (genau wie sie schon zuvor `viewFavoritesOnly` nie zurückgesetzt haben - gleiches, bereits etabliertes Muster, keine neue Inkonsistenz).
- Eigener Empty-State (Sparkle-Icon + "Aktuell keine Neuheiten." + Reset-Button `data-reset="new"`), eingehängt in dieselbe Ternary-Kette wie der bestehende Favoriten-Empty-State; `data-reset="new"` im bereits vorhandenen delegierten `[data-reset]`-Handler ergänzt.
- **Demo-Daten erweitert** (wie gefordert mit vorhandenem Mechanismus statt neuer Struktur): zusätzlich zu `p29` (Red Bull The Glacier Edition, bereits `isNew`) jetzt auch `p14` (Monster Ultra Watermelon) und `p16` (Rockstar Energy Punched Guava) auf `isNew:true` gesetzt, damit die neue Seite gemäß Anfrage-Beispielen ("neue Red Bull/Monster/Rockstar Sorten") mehrere Marken zeigt - insgesamt 5 Angebotskarten (die beiden neu markierten Produkte haben je 2 Angebote). Wirkt sich als Nebeneffekt auch auf den bereits bestehenden Sortier-Menü-Eintrag "Neuheiten" und die Burger-Menü-Ziehharmonika aus (beide zeigen jetzt ebenfalls 3 statt 1 Produkt/mehr Angebote) - gewünschter, konsistenter Nebeneffekt derselben Datenquelle, keine separate Änderung nötig.
- [CLAUDE.md](CLAUDE.md) um einen Absatz zur Bottom-Navigation ergänzt (welche Tabs echte separate Views sind vs. welche nur einen Filter auf `#deals` setzen).
- `CACHE_NAME` in `service-worker.js` auf `canspot-cache-v99` erhöht (Pflichtregel).

**Verifiziert** (mobil 375×812, Dark UND Light Mode, primär per gezielten JS-Klicks auf die echten Handler + Screenshots, da die Klick-Simulation der Vorschau in dieser Session wieder gelegentlich hing): "Neuheiten" zeigt korrekt 5 Angebotskarten über 3 Marken, Icon/Highlight in der Nav korrekt aktiv; Kombination mit Marken-Chip-Filter (z.B. "Monster") schränkt korrekt weiter ein; Favorisieren einer Neuheiten-Karte funktioniert (dieselbe Delegation wie überall); Wechsel zu "Favoriten" zeigt korrekt ALLE Favoriten (nicht auf Neuheiten beschränkt), `viewNewOnly` wird dabei korrekt zurückgesetzt; Wechsel zu "Alarme" und zurück zu "Neuheiten" funktioniert sauber; "Mein Bereich" öffnet sich wie gewohnt über der (weiterhin gefilterten) Neuheiten-Ansicht; bestehende Burger-Menü-Neuheiten-Übersicht zeigt konsistent 3 Einträge; Suchfeld im Banner weiterhin voll funktionsfähig (Eingabe + Vorschläge getestet) - die eigentliche Suche wurde nicht entfernt, nur der Bottom-Nav-Shortcut dorthin. Empty-State (Sparkle-Icon, Text, Reset-Button) per temporärem Daten-Mock verifiziert, Reset-Button setzt korrekt zurück auf "Start". Keine Konsolenfehler während des gesamten Tests.

**Hinweis zum Umfang**: Ausschließlich Mobile-Version bearbeitet und verifiziert, wie seit [[feedback-scope-mobile-only-default]] festgelegt - Desktop/Webapp nicht angefasst, nicht getestet.

---

## 2026-09-06 (52) — Suchleiste im Hero-Banner bleibt beim Scrollen oben sichtbar (Logo/Headline scrollen normal weg)

**Ziel**: Beim Scrollen soll nicht der komplette blaue Hero-Banner (Logo, "Dein Energy. Dein bester Preis.", Untertitel) mitscrollen bzw. oben hängen bleiben, sondern nur die Suchleiste inkl. eines schmalen blauen Hintergrundstreifens dahinter - Logo/Headline/Sub sollen ganz normal aus dem sichtbaren Bereich verschwinden und beim Zurückscrollen wieder normal erscheinen.

**Struktur**: `<header class="hero">` in zwei Teile aufgeteilt - `.hero-top` (Logo/Icons-Zeile + Headline + Sub, trägt jetzt die obere/seitliche Polsterung, die vorher am ganzen `.hero` hing) und `.hero-search-sticky` (nur die Suchleiste, trägt die untere Polsterung + den abgerundeten unteren Rand, den vorher der komplette Hero-Block hatte - dadurch bei Scroll-Position 0 optisch nicht vom bisherigen durchgehenden Banner zu unterscheiden). `.search`s eigenes `margin-top` entfernt und stattdessen als `padding-top` in `.hero-search-sticky` geführt (vermeidet Margin-Collapsing zwischen den beiden neuen Wrappern).

**Technische Klippe (der Grund, warum das kein simples `position:sticky` ist)**: Ein natives `position:sticky` auf `.hero-search-sticky` wurde zuerst versucht, griff aber nicht - Ursache ist `.app{overflow:hidden}` (existiert vermutlich fürs horizontale Clipping auf breiten/Desktop-Screens). `.app` hat nur `min-height:100vh` statt einer festen Höhe, wodurch **das Fenster** scrollt statt `.app` selbst - ein `overflow`-Vorfahre, der selbst nicht scrollt, verhindert nach CSS-Spec, dass sticky-Kinder greifen (systematisch isoliert und verifiziert: an einem Minimal-Repro sowie live per JS im DOM reproduziert; auch `overflow-x:hidden` allein reicht schon, da der Browser das fehlende `overflow-y` dabei automatisch auf `auto` setzt statt `visible` zu belassen - ebenfalls ein Scroll-Container). Da `.app` laut Auftrag nicht angetastet werden sollte (potenziell breiterer Blast-Radius auf ungetestete Bereiche), stattdessen: `position:fixed` (ignoriert `overflow` von Vorfahren komplett) + eigene JS-Logik (`updateHeroSearchSticky()`), die die `.is-stuck`-Klasse anhand von `window.scrollY` gegen die einmalig vermessene natürliche Position (`heroSearchThreshold = heroSearchSticky.offsetTop`) umschaltet, plus ein Spacer-Element (`#heroSearchSpacer`), das beim Umschalten auf `fixed` per JS auf die vorherige Höhe der Suchleiste gesetzt wird, damit der Rest der Seite (Filter-Chips, Angebote) nicht nach oben springt. Neu vermessen wird nur bei `resize`, nicht bei jedem Scroll (Höhe von `.hero-top` ändert sich nicht durch Angebotsdaten).

**Verifiziert** (mobil 375×812, Dark UND Light Mode, per echtem Scroll-Input über die Vorschau getestet, nicht nur programmatisch): Bei Scroll-Position 0 optisch identisch zum bisherigen Banner (kein Unterschied in Größen/Abständen/Farben). Herunterscrollen lässt Logo/Headline/Sub normal verschwinden, die Suchleiste (inkl. blauem Hintergrundstreifen mit abgerundetem unteren Rand) bleibt sauber oben hängen, kein Sprung im restlichen Layout. Weiter Herunterscrollen (auch tief in der Liste) hält die Suchleiste stabil oben. Hochscrollen bis 0 stellt den vollständigen Banner exakt wie ursprünglich wieder her. Suchleiste bleibt im "gestuckten" Zustand voll funktionsfähig (Eingabe + Vorschlagsliste getestet, korrekt darüber liegend). Ein währenddessen geöffnetes Sheet (Menü) legt sich weiterhin korrekt über die fixierte Suchleiste (z-index 20 für die Suchleiste, 40 für Overlays - unverändert ausreichend Abstand). Keine Konsolenfehler.
- `CACHE_NAME` in `service-worker.js` auf `canspot-cache-v98` erhöht (Pflichtregel).

**Hinweis zum Umfang**: Ausschließlich Mobile-Version bearbeitet und verifiziert, wie seit [[feedback-scope-mobile-only-default]] festgelegt - Desktop/Webapp nicht angefasst, nicht getestet. `.app{overflow:hidden}` selbst wurde bewusst NICHT verändert (siehe oben), um den Blast-Radius auf den neuen Bereich zu beschränken.
...
**Umsetzung**:
- `.period-count` (reiner Text) durch `.period-toggle` (Button) ersetzt: enthält jetzt ein Chevron-Icon (`#i-chevron-down`, bereits vorhandenes Icon-Symbol, wiederverwendet statt neu erstellt) + die Anzahl. `aria-expanded` am Button steuert sowohl die 180°-Drehung des Pfeils (CSS `transform`, exakt derselbe Rotations-Mechanismus wie beim bestehenden Menü-Akkordeon `.menu-acc-chevron`, zur Konsistenz wiederverwendet) als auch die Sichtbarkeit für Screenreader.
- Jede Wochen-Gruppe bekommt jetzt einen eigenen Wrapper `.period-group > .period-group-inner` um ihre Karten (Karten selbst unverändert, nur zusätzlich verschachtelt). `render()` baut jetzt IMMER alle Header UND alle Gruppen-Wrapper (nicht mehr nur die gerade sichtbare) — `periodWeekFilter` steuert stattdessen nur noch, welche `.period-group`s die Klasse `.collapsed` bekommen.
- Weiches Auf-/zuklappen über die CSS-Grid-Technik (`grid-template-rows: 1fr` ↔ `0fr` mit `overflow:hidden` im Inner-Element) statt der im bestehenden Menü-Akkordeon verwendeten `max-height`-Technik — bewusste Abweichung vom Präzedenzfall, weil die Wochen-Gruppen extrem unterschiedlich groß sind (aktuell 1 vs. 60 Angebote in den Demo-Daten); ein fixer `max-height`-Deckel, der groß genug für 60 Karten wäre, hätte die 1-Karten-Gruppe sichtbar zu schnell/unrund aufklappen lassen. Die Grid-Technik animiert dagegen zur tatsächlichen Inhaltshöhe, unabhängig von deren Größe — dadurch wirkt die Animation bei allen Gruppengrößen gleich "flüssig".
- Click-Verhalten (welche Woche isoliert wird, Umschalten, Zurücksetzen bei `groups.has(periodWeekFilter)===false`) unverändert aus (48) übernommen — nur die Präsentation (immer sichtbare Header, Animation, Pfeil-Icon) hat sich geändert.

**Verifiziert** (vor einer Session-Unterbrechung durch Nutzungslimit, danach anhand des gespeicherten Codes auf der Platte bestätigt, dass alle Änderungen erhalten geblieben sind): Klick auf "Diese Woche" klappt deren Karte auf, Pfeil dreht sich nach oben, "Nächste Woche"/"In 2 Wochen"-Header bleiben sichtbar mit nach unten zeigendem Pfeil und weich eingeklappten (0 Höhe) Karten-Bereichen; Umschalten auf eine andere Woche klappt die vorherige weich zu und die neue weich auf; erneuter Klick auf die aufgeklappte Woche klappt wieder alle auf (alle Pfeile zurück nach unten). Favoriten-Toggle nach dem DOM-Umbau (Karten jetzt in `.period-group-inner` verschachtelt statt direkte Kinder von `#deals`) erneut geprüft — funktioniert unverändert (Selektoren suchen ohnehin über alle Nachfahren, nicht nur direkte Kinder). Keine Konsolenfehler.
- [CLAUDE.md](CLAUDE.md) aktualisiert: `render()`-Beschreibung auf das jetzige Akkordeon-Verhalten (immer sichtbare Header, Grid-Animation, Chevron-Button) korrigiert.
- `CACHE_NAME` in `service-worker.js` auf `canspot-cache-v94` erhöht (Pflichtregel).

**Bekannter Blocker (Umgebung, nicht die App selbst)**: Der lokale Vorschau-Server (`python3 -m http.server`) sowie `git` schlagen auf diesem Rechner aktuell mit "You have not agreed to the Xcode license agreements" fehl — vermutlich hat ein automatisches Xcode-Command-Line-Tools-Update über Nacht die Lizenz-Bestätigung zurückgesetzt (beide Programme werden auf macOS über den Xcode-CLT-Stub bereitgestellt). Betrifft nur die lokale Werkzeugkette dieses Nutzers, nicht den App-Code. Fix (muss der Nutzer selbst interaktiv ausführen, braucht Passwort + Lizenz-Zustimmung): `sudo xcodebuild -license` in einem eigenen Terminal-Fenster.

**Hinweis zum Umfang**: Ausschließlich Mobile-Version bearbeitet, wie seit [[feedback-scope-mobile-only-default]] festgelegt.

---

## 2026-09-05 (48) — Wochen-Header als Filter anklickbar + "Neuheiten" an erste Stelle der Sortierung

**Wochen-Header-Klick**: Die Angebotsanzahl in einem Wochen-Abschnitts-Header (z.B. "1 Angebot" bei "Diese Woche", "60 Angebote" bei "Nächste Woche", siehe (47)) ist jetzt anklickbar (`data-period-toggle="<weekIndex>"`, `cursor:pointer`) und blendet alle anderen Wochen-Gruppen aus — wie eine Filteroption, aber ohne eigenes Sheet: neue State-Variable `periodWeekFilter` (`null` = alle Gruppen sichtbar, sonst der gewählte Wochenindex), gesetzt über einen delegierten Klick-Listener auf `dealsContainer` (derselbe Mechanismus wie bei `data-fav`/`data-hist` etc.). Erneuter Klick auf denselben (dann einzig sichtbaren) Header hebt die Einschränkung wieder auf. Bewusst eine eigene, neue Variable statt des bereits vorhandenen `periodFilter` (das ist ein anderes Konzept — die "Läuft bald ab"/"Neu"-Auswahl im Filter-Sheet, arbeitet auf `validFrom`/`validUntil`-Fenstern, nicht auf Wochen-Buckets). Verhält sich robust, falls die gefilterte Woche durch einen späteren Sortier-/Suchwechsel keine Angebote mehr enthält (Einschränkung wird dann für dieses Rendering einfach ignoriert, kein leerer Zustand). Die Kopfzeilen-Zusammenfassung ("62 Angebote" im Toolbar) bleibt bewusst unverändert und zeigt weiterhin die Gesamtzahl aller gefilterten Angebote — die Wochen-Einschränkung ist eine reine Anzeige-/Ansichtsebene obendrauf, vergleichbar mit dem Liste/Karte-Umschalter, der diese Zahl ebenfalls nicht verändert.

**Sortierung "Neuheiten" an erster Stelle**: Reihenfolge im `<select id="sort">` und im Sortier-Sheet geändert zu Neuheiten → Günstigster Preis → Nächster Händler → Beste Ersparnis → Endet bald. Da der Default-Sortierwert bisher implizit über die erste `<option>` ohne `selected`-Attribut lief, hätte die reine Umsortierung sonst das Anfangsverhalten der App geändert (Erststart hätte auf "Neuheiten" gefiltert, nur 1 statt 62 Angebote gezeigt) — daher zusätzlich `selected` explizit auf die "Günstigster Preis"-Option gesetzt, damit der tatsächliche Startzustand exakt wie zuvor bleibt und sich nur die Reihenfolge im Auswahlmenü ändert.

Verifiziert über einen temporären lokalen Server (Mobile): Klick auf "1 Angebot" bei "Diese Woche" zeigt korrekt nur diesen einen Karton, erneuter Klick stellt alle 3 Gruppen (62 Angebote) wieder her; dasselbe für "Nächste Woche" (60 Angebote); Zusammenspiel mit Sortierwechsel (inkl. "Neuheiten"-Sortierung) und dem Fallback bei nicht mehr vorhandener Wochen-Gruppe einzeln nachgestellt — funktioniert wie erwartet. Sortier-Sheet zeigt "Neuheiten" jetzt an erster Stelle, Start-Sortierung weiterhin "Günstigster Preis" (62 Angebote beim Laden). Keine Konsolenfehler.
- `CACHE_NAME` in `service-worker.js` auf `canspot-cache-v93` erhöht (Pflichtregel).

---

## 2026-09-05 (47) — Angebotsliste nach Angebotszeitraum gruppiert (Diese Woche / Nächste Woche / …)

**Analyse vor der Umsetzung**: Bestehenden Code untersucht statt neuer Datenstruktur — `deal.validFrom`/`deal.validUntil` (aus `deals.json`, bereits durchgereicht in `buildDeals()`) sowie `DEMO_TODAY`/`daysUntil()` waren schon vorhanden und ausreichend; keine zusätzlichen Felder oder eine parallele Datenquelle nötig.

**Umsetzung**: Zwei neue reine Hilfsfunktionen neben `daysUntil()`: `weekIndexOf(dateStr)` (rechnet ein Datum in einen Montag-basierten Wochenindex relativ zu `DEMO_TODAY` um — 0 = aktuelle Woche, 1 = nächste Woche, 2 = übernächste usw., rein arithmetisch aus dem Datum, keine Fallunterscheidungen) und `periodGroupLabel(weekIndex)` (liefert "Diese Woche"/"Nächste Woche"/"In N Wochen" — für Index ≥2 automatisch generisch, kein hartkodierter Text pro weiterer Woche). Die Karten-Erzeugung aus `render()` in eine eigene Funktion `buildDealCardEl(deal, isBest, favLiterMin)` ausgelagert (Markup/Logik 1:1 unverändert übernommen, nur aus der Schleife herausgezogen), damit sie sowohl einzeln als auch gruppiert aufgerufen werden kann. `render()` gruppiert die bereits fertig gefilterte/sortierte `filtered`-Liste anhand `weekIndexOf(deal.validUntil)` in ein `Map` (stabile Partitionierung, sortiert NICHT innerhalb einer Gruppe neu), rendert die Gruppen aufsteigend nach Wochenindex und überspringt dabei automatisch leere Gruppen. Vor jeder Gruppe wird ein `.period-header` als einfaches Geschwister-Element (kein Wrapper um die Karten) eingefügt: Titel + Anzahl ("N Angebot"/"N Angebote", automatisch aus der tatsächlichen Gruppengröße) in einer Zeile, Datumsspanne darunter kleiner/gedämpfter — die Spanne ist `formatDateRange(minValidFrom, maxValidUntil)` der tatsächlich in dieser Gruppe enthaltenen Angebote (nicht unabhängig berechnete Kalenderwoche), wie gefordert direkt aus den vorhandenen Angebotsdaten abgeleitet. Dezenter blauer vertikaler Akzentstrich links (`--brand-primary-blue`, dieselbe Farbe wie der Rest der App), keine neuen Flächenfarben. Zusätzliche mobile Media-Query-Regel verkleinert Titel/Anzahl/Datum leicht auf sehr schmalen Screens als Sicherheitsnetz gegen Umbruch/Kollision.

**Unverändert**: Karten-Markup/-Styling, Kartenansicht (`renderMap()` bekommt weiterhin dieselbe ungruppierte `filtered`-Liste), Favoriten-/Suche-/Filter-/Sortier-Logik, leerer Zustand, "Bester Preis"-Badge (weiterhin global über die gesamte gefilterte Liste berechnet, nicht pro Gruppe).

Verifiziert über einen temporären lokalen Server: Mobile (375×812) UND Desktop-Breite UND Light-Theme geprüft (Nutzer hat explizit responsive Darstellung auf beiden gefordert, abweichend vom sonstigen Mobile-only-Standardscope, siehe [[feedback-scope-mobile-only-default]] — hier ausnahmsweise beides getestet). Mit den echten Demo-Daten entstehen aktuell 3 Gruppen ("Diese Woche" 1 Angebot, "Nächste Woche" 60, "In 2 Wochen" 1 — Summe 62, stimmt mit der Gesamtzahl überein) — bestätigt, dass die Struktur über "nächste Woche" hinaus automatisch erweiterbar ist, ohne weiteren Code. Sortierung (inkl. "Neuheiten"-Filter, der korrekt nur die passende Gruppe zeigt und keine leere "Diese Woche"-Gruppe erzeugt), Kartenansicht-Umschalter, Favoriten-Tab und Favoriten-Toggle (mit frischer DOM-Abfrage nach jedem Re-Render) einzeln nachgetestet — alle unverändert funktionsfähig. Keine Konsolenfehler.
- [CLAUDE.md](CLAUDE.md) um die neue Gruppierungslogik in der "State & rendering"-Beschreibung ergänzt.
- `CACHE_NAME` in `service-worker.js` auf `canspot-cache-v92` erhöht (Pflichtregel).
...
**Stattdessen: die beiden konkret gemeldeten Bilder tatsächlich bearbeitet.** Analyse (Bilder lokal heruntergeladen und mit Pillow untersucht, siehe Vorgehen unten) ergab: Mehrpack-Packshots haben so gut wie nie ungenutzten Rand oben — bei `d59` (Monster Energy Ultra 24er) reicht die vergrößerte Einzeldose bis an den oberen Bildrand (Dosenrand/"ZERO SUGAR"-Schriftzug landet exakt unter dem Herz), bei `d51`/`d61` (Red Bull 4er-Pack) sitzt ein auf die Verpackung gedrucktes "4 PACK 250 ml"-Banner exakt in der oberen rechten Ecke. Keine Kombination aus Bildausschnitt/-parametern der Originalquelle (Amazon-Bildserver-Transforms bzw. REWE-CDN) konnte das beheben, ohne entweder ein falsches Foto zu liefern oder das Banner mit abzuschneiden.
- **Fix**: Beide Originalbilder heruntergeladen, mit Pillow (Python) oben (und etwas unten, zur optischen Balance) mit Weißraum/Transparenz aufgefüllt (Monster: +120px oben/+60px unten auf 225×225 Basis; Red Bull: +550px oben/+275px unten auf 840×840 Basis, danach auf 300px Breite verkleinert/quantisiert für vernünftige Dateigröße), sodass der durch `object-fit:contain` skalierte Bildinhalt komfortabel unterhalb der Herz-Ecke beginnt (rechnerisch ~35 % Rand oben statt der benötigten ~25 %, mit Sicherheitsabstand). Als neue lokale Dateien unter `images/bundles/monster-ultra-24pack.jpg` und `images/bundles/redbull-4pack.png` abgelegt, `deals.json`s `bundleImg` für `d51`/`d59`/`d61` auf diese relativen Pfade umgestellt (alle anderen `bundleImg`-Werte unverändert, weiterhin Hotlink).
- **Bewusste, eng begrenzte Ausnahme von der Hotlink-Regel**: in [CLAUDE.md](CLAUDE.md) (Abschnitt "Images") dokumentiert, warum genau diese zwei Bilder lokal statt gehotlinkt sind (Herz sitzt fix oben rechts ohne eigenen Hintergrund, offizielle Packshots haben dort teils unentfernbare Bild-/Verpackungsinhalte) — kein genereller Kurswechsel, nur für neu auftretende, sonst unlösbare Fälle dieser Art zu wiederholen. `deals.json`s `_meta.fields`-Doku für `bundleImg` entsprechend ergänzt.

Verifiziert über einen temporären lokalen Server (nur Mobile, 375×812), Service-Worker-Cache dafür geleert und neu aufgebaut: Herz auf allen Bundle-Karten wieder ohne sichtbaren Hintergrund/Schatten; Red-Bull-4er-Karte und Monster-Ultra-24er-Karte zeigen das Herz jetzt über echtem Weißraum, ohne Überlappung mit Dose/Banner; übrige, nicht gemeldete Bundle-Bilder (Red Bull 24er, 28 Black 24er, Rockstar 12er, Monster Classic/Pipeline Punch 12er) unverändert und weiterhin per Hotlink geladen; keine Konsolenfehler.
- `CACHE_NAME` in `service-worker.js` auf `canspot-cache-v84` erhöht (Pflichtregel).

**Hinweis zum Umfang**: Ausschließlich Mobile-Version bearbeitet und verifiziert, wie seit [[feedback-scope-mobile-only-default]] festgelegt — Desktop/Webapp nicht angefasst, nicht getestet, nicht erwähnt.

---

## 2026-09-05 (39) — Korrektur: "Neuheiten" jetzt reiner Filter (nur isNew), Herz-Icon auf Bundle-Karten nicht mehr vom Produktbild verdeckt

**"Neuheiten"-Korrektur**: (38) hatte "Neuheiten" so umgesetzt, dass zusätzlich zu `product.isNew` auch als "für den Händler untypisch" markierte Angebote (`unusualOffer`, z.B. Rockstar bei Penny) an vorderer Stelle auftauchten. Auf ausdrücklichen Wunsch ("dort sollen wirklich immer nur Sorten gelistet werden, die wir neu hinzufügen") ist das jetzt kein reines Um-Sortieren mehr, sondern ein echter Filter: `render()` schränkt die Liste bei `sort.value==="new"` auf `deal.isNew` ein (aktuell ausschließlich Red Bull The Glacier Edition, "1 Angebote"). Das komplette `unusualOffer`-Konzept aus (38) wieder entfernt: Feld raus aus `buildDeals()`, aus `deals.json` (Rockstar/Penny-Angebot `d26`) und aus der `_meta.fields`-Doku.

**Bug gefunden und behoben (Herz-Icon auf Mehrpack-Bildern)**: Auf mehreren Bundle-Karten (z.B. Red Bull 24×250ml, Monster Ultra 24×500ml, Red Bull 4er-Pack) lag das Favoriten-Herz optisch "in den Dosen" und war kaum zu erkennen. Ursache recherchiert und im Browser an den tatsächlichen Bildern verifiziert: Mehrpack-Packshots haben so gut wie immer irgendeine Kennzeichnung/Fülle genau in der oberen rechten Ecke, wo unser Herz-Icon sitzt — mal ist es ein echter Aufdruck auf der Verpackung selbst (z.B. "4 PACK 250 ml" bei den REWE-Bildern, Teil des offiziellen Packshots, nicht änderbar), mal ein von Amazon dynamisch eingeblendetes "24 pack"-Rundbadge (`_PIbundle-24,TopRight,0,0_`-Bildparameter), mal einfach die Dosen/der Tray, die bei dichter Produktfotografie bis an den Bildrand reichen. Ein Icon ohne eigenen Hintergrund verschmilzt in all diesen Fällen mit dem Bildinhalt.
- **Fix (robust, für alle Bilder)**: `.thumb-frame .btn.fav` bekommt jetzt einen deckenden Kreis-Hintergrund (`--bg-surface-elevated`, rund, mit Schatten) statt nur das nackte Icon — garantiert Lesbarkeit unabhängig vom jeweiligen Bildmotiv, in beiden Themes (der alte, nur-dark-mode Drop-Shadow-Filter wurde dadurch überflüssig und entfernt).
- **Zusätzlich, wo verifiziert sauber möglich**: `d59` (Monster Energy Ultra 24er bei Kaufland) auf eine bereinigte Amazon-Bild-URL ohne das künstliche "24 pack"-Badge umgestellt (`_AC_SX679_` statt `_PIbundle-24,TopRight,0,0_...`, gleiches Foto, Badge entfernt). Bei `d60` (28 Black Açaí 24er) bewusst NICHT am Bildparameter herumgedreht, da das Entfernen des Bundle-Parameters dort ein anderes (nur Einzeldose-)Foto liefert statt nur das Badge zu entfernen — hier greift ausschließlich der CSS-Fix.

Alles über einen temporären lokalen Server (nur Mobile, 375×812) verifiziert: "Neuheiten" zeigt jetzt exakt 1 Angebot (Glacier Edition); Herz-Icon auf allen geprüften Bundle-Karten (Red Bull 4er/24er, Monster Classic/Ultra/Pipeline Punch 12er/24er, Rockstar 12er, 28 Black 24er) klar sichtbar auf weißem/dunklem Kreis statt im Produktbild zu verschwimmen, in Light- UND Dark-Theme geprüft; Einzeldosen-Karten optisch unverändert (kein Regressions-Check-Befund).
- `CACHE_NAME` in `service-worker.js` auf `canspot-cache-v83` erhöht (Pflichtregel).
